    ...
    <client-only>
        <div>
            ...
            >
                ...
                <b-form-input v-model="messageInput" :placeholder='`Say hi to ${listing.userdata.firstname}`'></b-form-input>
            </b-modal>
                                    ...
                                    <div style='margin-right: 1rem; margin-left: 1rem'>
                                        <div v-if='listing.type === "sell"'>
                                            <span class='text-muted'>
                                                <b-icon v-b-tooltip.hover shift-v='-3' title='Selling' icon='box-arrow-up-right'></b-icon>
                                            </span>
                                        </div>
                                        <div v-else>
                                            <span class='text-muted'>
                                                <b-icon v-b-tooltip.hover  shift-v='-3'  title='Buying' icon='box-arrow-in-down-right'></b-icon>
                                            </span>
                                        </div>
                                    </div>
            ...
            <b-row>
                <b-col align='middle'>
                    <b-button @click='messageUser'><b-icon icon='envelope'></b-icon> Message {{listing.userdata.firstname}}</b-button>
                </b-col>
            </b-row>
        </div>
    </client-only>
...
<script>
    ...
    export default {
        ...
        async asyncData({params}) {
            ...
            if(listing.images){
                for (let j = 0; j < listing.images.length; j++) {
                    listing.images[j] = `https://click-clack.cc:5000/files/images/${listing.images[j]}`
                }
            }
            const userdata = await userService.getUser(listing.user)
        ...
        methods: {
            ...
            sendMessage() {
                messageService.sendMessage(this.user._id, this.listing.user, this.messageInput, this.token).then(() => {
        ...
        head() {
            ...
            let image = 'https://click-clack.cc:5000/files/images/indeximage.JPG'
            if(k.images){
                image = k.images[0]
            }
