  ...
  <div>
    ...
    <b-row>
      <b-col>
        <b-row>
          ...
          <b-col>
            Preview
            <div v-html="$md.render( announcement?announcement:``)" />
          </b-col>
        </b-row>
    ...
    <b-card class="dataListCard">
      ...
      >
        ...
        <template v-slot:cell(_id)="data">
          <b-button size="sm" @click="$nuxt.$router.push(`/post/${data.value}`)">
            Visit
          </b-button>
        </template>
      </b-table>
      ...
      >
        ...
        <template v-slot:cell(state)="data">
          ...
          <span v-if="data.value.state === 'pending'">
            <b-button variant="success" @click="approveListing(data.value.id)">
              APPROVE
            </b-button>
            <b-button variant="danger" @click="declineListing(data.value.id)">
              DECLINE
            </b-button>
          </span>
        </template>
        ...
        <template v-slot:cell(_id)="data">
          <b-button size="sm" @click="$nuxt.$router.push(`/listing/${data.value}`)">
            Visit
          </b-button>
        </template>
      </b-table>
    ...
    <b-card class="dataListCard">
      ...
      >
        ...
        <template v-slot:cell(_id)="data">
          <b-button size="sm" @click="$nuxt.$router.push(`/keyboard/${data.value}`)">
            Visit
          </b-button>
        </template>
      </b-table>
    ...
    <b-card class="dataListCard">
      ...
      >
        ...
        <template v-slot:cell(idcopy)="data">
          <b-button size="sm" @click="$nuxt.$router.push(`/u/${data.value}`)">
            Visit
          </b-button>
        </template>
      </b-table>
...
export default {
	...
	async asyncData ({ app }) {
		return {
			posts: await app.$services.postService.getNewPosts('new'),
			listings: [],
			keyboards: await app.$services.keyboardService.getNewKeyboards('new'),
			users: (await app.$services.userService.getNewUsers()).map(user => ({ ...user, idcopy: user.id }))
		}
	},
	data () {
		return {
	...
	methods: {
		getAdminLimitedStuff () {
			if (!this.user) { return }
			this.$services.listingService.getNewListingsForMod(this.user._id, this.token).then((res) => {
		...
		submitAnnouncement () {
			...
			this.$services.announcementService.sendAnnouncement(this.user._id, this.announcement, this.token).then((res) => {
		...
		approveListing (id) {
			this.$services.listingService.approve(this.user._id, id, this.token).then((res) => {
		...
		declineListing (id) {
			this.$services.listingService.decline(this.user._id, id, this.token).then((res) => {
